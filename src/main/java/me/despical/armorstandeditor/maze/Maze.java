...
public class Maze {
    ...
    // this class was for making 2D maze with strings, then implemented to minecraft without changing variables / x = x, y = z, y = ?
    ...
    public Maze(int xDimension, int yDimension) {
        ...
        grid = new Material[gridDimensionX][gridDimensionY];
        init();
        generateMaze();
    }
    ...
    public void updateGrid() {
        Material backChar = Material.AIR, wallChar = Material.SANDSTONE, cellChar = Material.AIR, pathChar = Material.DIAMOND_BLOCK;
    ...
    public void build(Location location, int height) {
        updateGrid();

        for (int y = 0; y < gridDimensionY; y++) {
            for (int x = 0; x < gridDimensionX; x++) {
                for (int ny = 0; ny < height; ny++)
                    location.clone().add(x, ny, y).getBlock().setType(grid[x][y]);
            }
        }
    }
}